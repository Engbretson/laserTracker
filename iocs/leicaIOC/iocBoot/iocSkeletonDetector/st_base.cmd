...
#dbLoadDatabase("$(ADleica)/dbd/windows-x64/leicaApp.dbd")
...
# Create a leica driver
# leicaConfig(const char *portName, int maxSizeX, int maxSizeY, int dataType,
...
dbLoadRecords("$(ADleica)/db/leica.template","P=$(PREFIX),R=cam1:,PORT=$(PORT),ADDR=0,TIMEOUT=1")

# Load an NDFile database.  This is not supported for the leica which does not write files.
#dbLoadRecords("NDFile.template","P=$(PREFIX),R=cam1:,PORT=SIM1,ADDR=0,TIMEOUT=1")

# Create a standard arrays plugin, set it to get data from first leica driver.
...
set_requestfile_path("$(ADleica)/leicaApp/Db")
